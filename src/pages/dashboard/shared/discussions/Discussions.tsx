...
const Discussions: React.FC = () => {
    ...
    const { discussions, replies } = useDiscussions(subjectId);

    const [searchQuery, setSearchQuery] = useState('');
    const [filter, setFilter] = useState<'all' | 'unread' | 'subscribed'>('all');
    ...
    const filteredDiscussions = discussions.filter(d => {
        ...
        if (filter === 'unread' && user) {
            const isDiscussionUnread = !d.readByUsers.includes(user.id);
            const hasUnreadReplies = getUnreadRepliesCount(d.id) > 0;
            return matchesSearch && (isDiscussionUnread || hasUnreadReplies);
        }
        if (filter === 'subscribed' && user) {
            return matchesSearch && d.subscribedUserIds.includes(user.id);
        }
    ...
    const DiscussionItem = ({ discussion }: { discussion: any }) => {
        ...
        const totalCount = getRepliesCount(discussion.id);
        const isUnread = user && !discussion.readByUsers.includes(user.id);
